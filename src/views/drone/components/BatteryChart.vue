  ...
  return {
    ...
    yaxis: [
      ...
      },
      {
        seriesName: 'Voltage',
        opposite: true,
        title: { text: 'V', style: { color: labelColor.value } },
        labels: { style: { fontSize: '11px', colors: labelColor.value } },
        axisBorder: { show: true, color: axisColor.value }
      }
    ],
    ...
    tooltip: {
      ...
      custom: ({ series, dataPointIndex }: { series: number[][], dataPointIndex: number }) =>
        `<div class="q-pa-sm">
          <div class="text-caption">Charge: <b>${series[0][dataPointIndex]}</b> %</div>
          <div class="text-caption">Voltage: <b>${series[1][dataPointIndex]}</b> V</div>
        </div>`
    }
  }
})
