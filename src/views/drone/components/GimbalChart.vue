...
const series = computed(() => {
  ...
  return [
    { name: 'Pitch', data: sampled.value.map((f, i) => ({ x: times[i], y: parseFloat(f.gimbal.pitch.toFixed(2)) })) },
    { name: 'Yaw',   data: sampled.value.map((f, i) => ({ x: times[i], y: parseFloat(f.gimbal.yaw.toFixed(2)) })) }
  ]
})
...
const options = computed<ApexOptions | null>(() => {
  if (!sampled.value.length) return null
  return {
    chart: { type: 'line', height: 280, id: 'gimbal-chart', toolbar: { show: false }, animations: { enabled: false } },
    ...
    tooltip: {
      ...
      custom: ({ series, dataPointIndex }: { series: number[][], dataPointIndex: number }) =>
        `<div class="q-pa-sm">
          ...
          <div class="text-caption">Yaw: <b>${series[1][dataPointIndex]}°</b></div>
        </div>`
    }
  }
})
...
<template>
  <q-card v-if="options" flat bordered class="rounded-borders">
    <q-card-section class="row items-center q-pb-sm">
      <q-icon name="mdi-camera-flip-outline" color="primary" size="22px" class="q-mr-sm" />
      <span class="text-subtitle1 text-primary text-weight-bold">Gimbal</span>
    </q-card-section>
    <q-separator />
