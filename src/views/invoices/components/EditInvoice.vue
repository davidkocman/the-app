...
// types
import type { SavedCompany, SavedInvoice, InvoiceItem } from '@/types/invoices'
...
const tableHeaders = ref<QuasarTableHeader[]>([
  { name: 'name', required: true, label: 'Názov položky', align: 'left', field: 'name', sortable: false },
...
watch(
  ...
  (val) => {
    if (val) {
      deliveryDate.value = props.invoice.deliveryDate
      dueDate.value = props.invoice.dueDate
      issueDate.value = props.invoice.issueDate
      variableSymbol.value = props.invoice.variableSymbol
      zeroVatRateValue.value = props.invoice.zeroVatRateValue
      selectedConsumer.value =
        companies.value.find((c) => c.id === props.invoice.consumer.id) || props.invoice.consumer
      selectedSupplier.value =
        companies.value.find((c) => c.id === props.invoice.supplier.id) || props.invoice.supplier
      vatRate.value = props.invoice.invoiceItems[0]?.vatRate ?? 0
      tableRows.value = props.invoice.invoiceItems.map((item) => ({ ...item }))
      index.value = 0
    }
  }
)
...
const vat = computed(() => {
  return ((Number(basePrice.value) * (tableRows.value[0]?.vatRate ?? 0)) / 100).toFixed(2)
})
const canSave = computed(() => {
  if (tableRows.value.some((item) => item.vatRate === 0) && !zeroVatRateValue.value) return false
  return true
})

const addInvoiceItem = () => {
  tableRows.value.push({ name: '- Názov položky -', quantity: 1, unit: '', price: 0, vatRate: vatRate.value, vatPrice: 0 })
}
...
const saveInvoice = () => {
  invoiceStore.editInvoice(props.invoice.id, {
...
<template>
  ...
  >
    <q-card flat class="invoice-heading">
      <q-bar>
        <span class="text-subtitle2">Edit Invoice — FA{{ invoice.variableSymbol }}</span>
        <q-space />
      ...
      <q-card-section>
        <div class="section-wrapper shadow-2 q-pa-sm rounded-borders">
          <div class="row q-mb-sm q-gutter-md">
            <div class="col">
              <q-select outlined v-model="selectedConsumer" :options="companies" option-label="name" option-value="id" label="Odberateľ" />
            </div>
            <div class="col">
              <q-select outlined v-model="selectedSupplier" :options="companies" option-label="name" option-value="id" label="Dodávateľ" />
            </div>
          </div>
          <div class="row q-gutter-md">
            <div class="col">
              <div v-if="selectedConsumer" class="consumer-details q-pl-sm">
                <h4 class="text-caption">{{ selectedConsumer.street }}</h4>
                <h4 class="text-caption">{{ selectedConsumer.zip }}, {{ selectedConsumer.city }}</h4>
                <h4 class="text-caption">{{ selectedConsumer.country }}</h4>
                <q-separator class="q-my-sm" />
                <h4 class="text-caption">IČO: <span class="text-subtitle2">{{ selectedConsumer.companyId }}</span></h4>
                <h4 class="text-caption">DIČ: <span class="text-subtitle2">{{ selectedConsumer.taxId }}</span></h4>
                <h4 v-if="selectedConsumer.vatId" class="text-caption">IČ DPH: <span class="text-subtitle2">{{ selectedConsumer.vatId }}</span></h4>
              </div>
            </div>
            <div class="col">
              <div v-if="selectedSupplier" class="consumer-details q-pl-sm">
                <h4 class="text-caption">{{ selectedSupplier.street }}</h4>
                <h4 class="text-caption">{{ selectedSupplier.zip }}, {{ selectedSupplier.city }}</h4>
                <h4 class="text-caption">{{ selectedSupplier.country }}</h4>
                <q-separator class="q-my-sm" />
                <h4 class="text-caption">IČO: <span class="text-subtitle2">{{ selectedSupplier.companyId }}</span></h4>
                <h4 class="text-caption">DIČ: <span class="text-subtitle2">{{ selectedSupplier.taxId }}</span></h4>
                <h4 v-if="selectedSupplier.vatId" class="text-caption">IČ DPH: <span class="text-subtitle2">{{ selectedSupplier.vatId }}</span></h4>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="section-wrapper shadow-2 q-pa-sm rounded-borders">
          <div class="row q-gutter-md">
            <div class="col-10 col-md-6">
              ...
              <div class="row q-gutter-md q-mb-md">
                <div class="col">
                  <q-input outlined v-model="variableSymbol" label="Variabilný symbol" />
                </div>
              </div>
            </div>
            ...
            <template v-slot:body="props">
              <q-tr :props="props">
                ...
                <q-td key="vatRate" :props="props">
                  {{ props.row.vatRate }}
                  <q-popup-edit v-model="props.row.vatRate" title="DPH%" buttons v-slot="scope" :validate="minMaxVatRateValidation">
                    <q-input
                      type="number"
                      v-model.number="scope.value"
                      dense
                ...
                <q-td v-if="tableRows.length > 1" :props="props" key="actions">
                  <q-btn color="negative" icon="delete" no-caps flat dense @click="removeRow(tableRows.indexOf(props.row))" />
                </q-td>
              </q-tr>
            </template>
          </q-table>
